...
import { API_CONFIG_STORE_KEY } from '@/lib/user-session';
...
export const AI_FEATURES: Array<{
  ...
  { key: 'promotion_planning', name: '品宣规划', description: '分析产品图和创作意图，生成产品档案与卖点分镜' },
  { key: 'script_analysis', name: '剧本分析', description: '将故事文本分解为结构化剧本' },
  { key: 'character_generation', name: '兼容图片生成', description: '保留旧数据兼容，不在品宣设置中展示' },
  ...
  { key: 'freedom_image', name: '品宣生图', description: '为宣传图节点生成最终海报图' },
...
function createInitialAPIConfigState(): APIConfigState {
  return {
    providers: [],
...
const initialState: APIConfigState = createInitialAPIConfigState();
...
export const useAPIConfigStore = create<APIConfigStore>()(
  persist(
    (set, get) => ({
      ...
      resetConfig: () => {
        clearAllManagers();
        set(createInitialAPIConfigState());
      },
    ...
    {
      name: API_CONFIG_STORE_KEY,
      storage: createJSONStorage(() => createUserScopedStorage(API_CONFIG_STORE_KEY)),
      skipHydration: true,
