...
const FEATURE_CONFIGS: FeatureMeta[] = [
  {
    key: "promotion_planning",
    name: "品宣规划",
    description: "分析产品图和创作意图，生成产品报告与宣传图节点",
    icon: <Sparkles className="h-4 w-4" />,
    requiredCapability: "text",
    recommendation: "建议选择支持图片输入/多模态理解的对话模型；只支持文本的模型无法分析上传的产品图",
  },
  {
    key: "freedom_image",
    name: "品宣生图",
    description: "为宣传图节点生成最终海报图；不与其他模块共用映射",
    icon: <Sparkles className="h-4 w-4" />,
    requiredCapability: "image_generation",
    recommendation: "建议绑定 gpt-image / images2.0 / Gemini 图片模型；参考图失败时会直接报错，不做纯文本兜底",
  },
];
...
export function FeatureBindingPanel() {
  ...
  }, [featureBindings, optionsByFeature, configuredProviderIds, getFeatureBindings]);
  ...
  return (
    <div className="w-full min-w-0 overflow-hidden space-y-4 rounded-lg border border-border bg-card p-3 sm:space-y-6 sm:rounded-xl sm:p-6">
        ...
        {FEATURE_CONFIGS.map((feature) => {
          ...
          return (
            <div
              key={feature.key}
              className={cn(
                "min-w-0 overflow-hidden rounded-lg border transition-all",
                configured
            ...
            >
              ...
              <div 
                className="flex min-w-0 cursor-pointer items-start gap-3 p-3 transition-colors hover:bg-accent/50 sm:items-center sm:gap-4 sm:p-4"
                ...
                <div className="flex min-w-0 flex-1 items-start gap-3 sm:items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex min-w-0 flex-wrap items-center gap-2">
                      <Label className="shrink-0 cursor-pointer font-medium text-foreground">
                        {feature.name}
                      </Label>
                      {configured ? (
              ...
              {isExpanded && (
                <div className="border-t border-border/50 px-3 pb-3 pt-0 sm:px-4 sm:pb-4">
                  {options.length === 0 ? (
                    ...
                    <div className="space-y-3 pt-3">
                      ...
                      {isFreedomFeature && (
                        <div className="flex flex-col gap-2 rounded-md border border-border/60 bg-muted/30 px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                filteredOptions.map((option) => {
                                  ...
                                  return (
                                    <label
                                      key={optionKey}
                                      className={cn(
                                        "flex min-w-0 items-center gap-3 rounded-md p-2 cursor-pointer transition-colors",
                                        isSelected
                                    ...
                                    >
                                      ...
                                      {feature.key === "promotion_planning" && (
                                        <span className={cn(
